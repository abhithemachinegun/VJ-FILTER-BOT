...
async def start(client, message):
    ...
    if message.chat.type in [enums.ChatType.GROUP, enums.ChatType.SUPERGROUP]:
        buttons = [[
            ...
            InlineKeyboardButton('✪ Sᴜᴘᴘᴏʀᴛ Gʀᴏᴜᴘ', url="https://t.me/titaniummovieflix"),
            InlineKeyboardButton('⌬ Mᴏᴠɪᴇ Gʀᴏᴜᴘ', url='https://t.me/atozmoviesflix')
        ],[
    ...
    if len(message.command) != 2:
        ...
        reply_markup = InlineKeyboardMarkup(buttons)
        m=await message.reply_sticker("CAACAgUAAxkBAAEH649mdj_DPcWMdoo3gvaDK2CPpCJnewACMwYAAuLuyVTPyIqqHhlqCx4E") 
            ...
            try:
                ...
                if STREAM_MODE == True:
                    button = [[
                        ...
                        InlineKeyboardButton('Join Whatsapp Channel', url="https://whatsapp.com/channel/0029VahoJUGAInPqCX4dK80M")
                    ],[
        ...
        for file in files:
            ...
            if not await db.has_premium_access(message.from_user.id):
                if not await check_verification(client, message.from_user.id) and VERIFY == True:
                    btn = [[
                        InlineKeyboardButton("Premium Benifits", url="https://t.me/premium_advantage/2")
                    ],[
                        InlineKeyboardButton("Send Payment Screenshot", url="https://t.me/Player001robot")
                    ]]
                    await message.reply_text(
                        text="<b>Buy premium to get movies!\nClick 👉/plan to buy premium </b>",
                        protect_content=True,
                        reply_markup=InlineKeyboardMarkup(btn)
                    )
                    return
    ...
    if not files_:
        ...
        try:
            ...
            title = '@atozmoviesflix  ' + ' '.join(filter(lambda x: not x.startswith('[') and not x.startswith('@'), file.file_name.split()))
    ...
    files = files_[0]
    title = '@atozmoviesflix  ' + ' '.join(filter(lambda x: not x.startswith('[') and not x.startswith('@'), files.file_name.split()))
    ...
    if CUSTOM_FILE_CAPTION:
        try:
            ...
            f_caption=f_caption
    if f_caption is None:
        f_caption = f"@atozmoviesflix  {' '.join(filter(lambda x: not x.startswith('[') and not x.startswith('@'), files.file_name.split()))}"
    if not await db.has_premium_access(message.from_user.id):
        if not await check_verification(client, message.from_user.id) and VERIFY == True:
            btn = [[
                InlineKeyboardButton("Premium Benifits", url="https://t.me/premium_advantage/2")
